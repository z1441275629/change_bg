(function () {
  ...
  Clip.prototype = {
    init() {
      ...
      this.opt.initialHeight =
        this.opt.initialHeight < 30
          ? 100
          : this.opt.initialHeight < this.opt.minHeight
          ? this.opt.minHeight
          : this.opt.initialHeight;
      this.opt.initialWidth =
        this.opt.initialWidth < 30
          ? 100
          : this.opt.initialWidth < this.opt.minWidth
          ? this.opt.minWidth
          : this.opt.initialWidth;
    ...
    setSize(files) {
      ...
      img.onload = function () {
        ...
        if (!(_this.opt.maxWidth && _this.opt.maxWidth >= 30) && _this.opt.maxHeight && _this.opt.maxHeight >= 30) {
          ...
          !(_this.opt.maxHeight && _this.opt.maxHeight >= 30) &&
          _this.opt.maxWidth &&
          _this.opt.maxWidth >= 30
        ) {
          if (_this.opt.clipRadio) {
          ...
          !(_this.opt.maxWidth && _this.opt.maxWidth >= 30) &&
          !(_this.opt.maxHeight && _this.opt.maxHeight >= 30)
        ) {
          if (_this.opt.clipRadio) {
    ...
    up(e) {
      ...
      if (e.clientY - offset.top - blockOriginal.height <= blockOriginal.top - sTop) {
        ...
        var height = Math.min(
            e.clientY - offset.top - blockOriginal.top - blockOriginal.height + sTop,
      ...
      if (e.clientX - offset.left - blockOriginal.width - blockOriginal.left <= 0) {
        var left = Math.max(e.clientX - offset.left, 0),
          width = Math.min(
            ...
            blockOriginal.left + blockOriginal.width - (e.clientX - offset.left)
          );
      } else {
        var width = Math.min(
            e.clientX - offset.left - blockOriginal.left - blockOriginal.width,
    ...
    finished() {
      var _this = this,
        img = {
          ...
          top: _this.opt.top,
          left: _this.opt.left,
          o_width: _this.imgOrginWidth,
          o_height: _this.imgOrginHeight,
        },
